...
export function SmartInputFields({ metadata, onMetadataChange, promptText }: SmartInputFieldsProps) {
  ...
  const currentUseCase = metadata?.use_case || "";
  ...
  const setUseCase = (useCase: string) => {
    onMetadataChange({
      ...metadata,
      use_case: useCase
    });
  ...
  return (
    <div className="space-y-6 bg-gradient-to-br from-blue-50 to-purple-50 p-6 rounded-xl border border-blue-200">
      ...
      <div className="space-y-3">
        ...
        />
        <div className="space-y-2">
          <div className="text-xs text-gray-600 font-medium">Common use cases:</div>
          <div className="flex flex-wrap gap-2">
            {suggestions.useCases.map((useCase, index) => (
              <Button
                key={index}
                type="button"
                variant="outline"
                ...
                className="text-xs h-7 px-2 hover:bg-green-100 hover:border-green-300"
              >
                {useCase}
              </Button>
            ))}
