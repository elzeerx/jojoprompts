      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Button asChild size="sm" className="min-h-[44px] bg-warm-gold text-dark-base hover:bg-warm-gold/90">
            <Link to="/admin/content?tool=new">New Resource</Link>
          </Button>
        </div>
      </header>
                  ...
                  <td className="px-3 py-2 align-top">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem asChild><Link to={`/admin/content?tool=edit&resourceId=${encodeURIComponent(r.id)}`}><Pencil className="me-2 h-3.5 w-3.5" /> Edit</Link></DropdownMenuItem>
                        <DropdownMenuItem asChild><Link to={`/admin/content?tool=new-version&resourceId=${encodeURIComponent(r.id)}`}><GitBranch className="me-2 h-3.5 w-3.5" /> New version</Link></DropdownMenuItem>
                ...
                <div key={r.id} className="flex flex-col justify-between rounded-md border border-gray-200 bg-white p-3">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Button asChild size="sm" variant="outline" className="min-h-[44px]">
                      <Link to={`/admin/content?tool=edit&resourceId=${encodeURIComponent(r.id)}`}><Pencil className="me-1 h-3.5 w-3.5" /> Edit</Link>
                    </Button>
                    <Button asChild size="sm" variant="outline" className="min-h-[44px]">
      ...
      <Dialog open={!!publishResult} onOpenChange={(v) => (v ? null : setPublishResult(null))}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            {publishResult ? (
              <Button asChild><Link to={`/admin/content?tool=edit&resourceId=${encodeURIComponent(publishResult.resourceId)}`}>Open publisher</Link></Button>
            ) : null}
          </DialogFooter>
        </DialogContent>
      </Dialog>
