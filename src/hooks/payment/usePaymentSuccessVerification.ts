...
export function usePaymentSuccessVerification({
    ...
    try {
      ...
      if (user && data.transaction && data.plan) {
        ...
        if (profile && user.email) {
          ...
          // Send email in background to avoid blocking the success flow
          setTimeout(async () => {
  ...
  }, [params.paymentId, params.orderId, params.planId, params.userId, params.debugObject, user, setVerifying, setVerified, setError, sendPaymentConfirmation]);
