...
/** Page size for the paginated events table. */
export const SECURITY_EVENTS_PAGE_SIZE = 50;

/**
 * Actions considered "routine noise". Hidden from the default list and
 * excluded from the 24h Total Events count unless the admin opts in.
 */
...
/**
 * Pure helper — read query params into a normalized filter set. Exported
 * for the focused filter-mapping test.
 */
export interface SecurityEventsFilters {
  page: number;
  severity: (typeof SEVERITY_OPTIONS)[number];
  category: (typeof CATEGORY_OPTIONS)[number];
  action: string; // free text; "all" or specific slug
  q: string;
...
export function parseSecurityEventsFilters(
  ...
  return {
    page,
    severity: severity && (SEVERITY_OPTIONS as readonly string[]).includes(severity)
      ? severity
      : "all",
    category: category && (CATEGORY_OPTIONS as readonly string[]).includes(category)
      ? category
      : "all",
...
/**
 * Build the "since 24h ago" ISO string. Exported so tests can assert
 * the metric period label and query bound.
 */
