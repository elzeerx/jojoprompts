      ...
      <DialogContent className="max-w-2xl max-h-[85vh] p-0" aria-describedby="prompt-details-description">
        <ScrollArea className="h-full max-h-[85vh]">
          <div className="p-6 flex flex-col space-y-6">
            ...
            {imagePath ? <div className="rounded-xl overflow-hidden max-w-full mx-auto">
                <ImageWrapper src={dialogImgUrl} alt={prompt.title} aspect={16 / 9} className="cursor-zoom-in transition-all duration-300" onClick={() => {
              if (imagePath) {
                const fullImage = getPromptImage(imagePath, 2000, 100);
                if (fullImage) window.open(fullImage, "_blank", "noopener,noreferrer");
              }
            }} />
              </div> : <div className="rounded-xl overflow-hidden bg-muted/50 flex items-center justify-center h-48">
