...
-- =========================================================================
-- 1) Additive resource fields (EN/AR): examples, limitations, uninstall,
--    support, update info. Nullable, safe on existing rows.
-- =========================================================================
ALTER TABLE public.resources
  ADD COLUMN IF NOT EXISTS examples_en    text,
  ADD COLUMN IF NOT EXISTS examples_ar    text,
  ADD COLUMN IF NOT EXISTS limitations_en text,
  ADD COLUMN IF NOT EXISTS limitations_ar text,
  ADD COLUMN IF NOT EXISTS uninstall_en   text,
  ADD COLUMN IF NOT EXISTS uninstall_ar   text,
  ADD COLUMN IF NOT EXISTS support_en     text,
  ADD COLUMN IF NOT EXISTS support_ar     text,
  ADD COLUMN IF NOT EXISTS update_info_en text,
  ADD COLUMN IF NOT EXISTS update_info_ar text;

-- =========================================================================
-- 2) save_admin_resource_draft — full replacement.
--    Immutable versioning, ownership-safe SKUs, no per-resource lifetime,
--    bundle-item hygiene, admin-only.
-- =========================================================================
...
DECLARE
  ...
  v_current_is_published boolean := false;
...
BEGIN
  ...
  IF v_new_resource THEN
    INSERT INTO public.resources (
      ...
      v_slug, v_type::public.resource_type, 'draft',
  ...
  ELSE
    ...
    -- Lock the row so concurrent version promotion cannot race us
    ...
    -- Cannot rename slug to one owned by another resource
    ...
    UPDATE public.resources SET
      slug = v_slug,
      type = v_type::public.resource_type,
  ...
  -- ------------------------------------------------------------------
  -- Version handling (immutable when current is published)
  -- ------------------------------------------------------------------
  IF v_ver IS NOT NULL THEN
    ...
    -- Reload current version info under lock (skip fresh insert path)
    ...
    IF v_current_version_id IS NOT NULL THEN
      SELECT (published_at IS NOT NULL) INTO v_current_is_published
        FROM public.resource_versions WHERE id = v_current_version_id;
    END IF;

    IF v_want_new_version OR v_current_is_published OR v_current_version_id IS NULL THEN
      -- Reject duplicate (resource_id, version)
      IF EXISTS (
      ...
      -- Insert new version NOT current
      ...
      -- Demote existing current (if any), then promote the new row
      IF v_current_version_id IS NOT NULL THEN
    ...
    ELSE
      -- Safe to update in place (draft, not published)
  ...
  -- ------------------------------------------------------------------
  -- Platform compatibility: replace snapshot
  -- ------------------------------------------------------------------
  ...
  -- Installation guides: replace snapshot
  ...
  IF payload ? 'permissions' THEN
    ...
    SELECT
      v_resource_id,
      (row ->> 'kind')::public.permission_kind,
      row ->> 'key',
      NULLIF(row ->> 'label_en',''),
  ...
  -- ------------------------------------------------------------------
  -- Products: strict integrity rules
  -- ------------------------------------------------------------------
  IF payload ? 'products' THEN
    -- Validate every incoming row up front before mutating
    ...
    LOOP
      ...
      -- SKU ownership: never steal a SKU from another resource
    ...
    -- Deactivate products for this resource not in incoming set (soft removal)
    ...
    LOOP
      ...
      ELSE
        INSERT INTO public.products (
          ...
          v_sku, (v_prod ->> 'product_type')::public.product_type, v_resource_id,
  ...
  -- ------------------------------------------------------------------
  -- Bundle items (only for bundle resources) — dedupe and reject self
  -- ------------------------------------------------------------------
  IF v_type = 'bundle' AND payload ? 'bundle_items' THEN
    ...
    BEGIN
      ...
      IF v_bundle_product_id IS NULL THEN
        -- Do not auto-create; publish will complain if absent.
        NULL;
      ELSE
...
-- =========================================================================
-- 3) admin_publish_resource — strengthened
-- =========================================================================
...
BEGIN
  ...
  IF v_r.type IN ('skill','automation') THEN
    ...
    END IF;
    -- Require at least one file and clean scan on the current version
  ...
  IF v_r.type = 'bundle' THEN
    SELECT count(*) INTO v_positive_bundle_products
     ...
     WHERE resource_id = p_resource_id
       AND product_type = 'bundle' AND is_active AND price_fils > 0;
...
-- =========================================================================
-- 4) admin_transition_resource_lifecycle — hardened
-- =========================================================================
...
BEGIN
  ...
  UPDATE public.resources
     SET lifecycle = v_new_lifecycle::public.resource_lifecycle,
...
-- =========================================================================
-- 5) get_admin_v2_overview — corrected payment counting + scan attention
-- =========================================================================
...
BEGIN
  ...
  FROM public.orders
  WHERE status = 'paid' AND settled_at >= v_since;

  SELECT count(*) INTO v_failed_orders_period
  FROM public.orders WHERE status = 'failed' AND updated_at >= v_since;
  ...
  SELECT
    count(*) FILTER (WHERE grant_reason = 'lifetime_purchase'),
    count(*) FILTER (WHERE grant_reason = 'lifetime_threshold')
  INTO v_lifetime_purchases, v_lifetime_threshold
  FROM public.entitlements
  WHERE scope = 'lifetime';

  -- Latest terminal payment state per order in period — dedupe authorized+captured
  WITH latest AS (
  ...
  SELECT count(*), count(*) FILTER (WHERE lifecycle = 'published')
    INTO v_total_resources, v_published_resources FROM public.resources;

  SELECT count(*) FILTER (WHERE lifecycle = 'draft'),
         count(*) FILTER (WHERE lifecycle = 'review')
    INTO v_drafts, v_review FROM public.resources;

  -- Scan attention: current versions that are non-clean OR carry files but no scan yet
  ...
  LEFT JOIN LATERAL (
    SELECT status::text AS s
     ...
     ORDER BY coalesce(ps.scanned_at, ps.created_at) DESC LIMIT 1
  ) last_scan ON true
  WHERE r.type IN ('skill','automation')
  ...
  SELECT count(*) INTO v_pending_refunds
    FROM public.refunds WHERE status = 'requested';
...
-- =========================================================================
-- 6) Lock down execute grants — admin auth is enforced inside each function.
-- =========================================================================
