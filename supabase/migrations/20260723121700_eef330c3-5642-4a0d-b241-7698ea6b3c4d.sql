...
-- 1) Fix two-arg service-only overload: honor collection scope + collection_key
...
-- Two-arg overload remains service-only (never REST-callable).
...
-- 2) Rewrite preview RPC: fix array bug, strict mappings, richer aggregates.
...
AS $function$
DECLARE
  v_rate         numeric := 307.55;   -- 1 USD = 307.55 fils (fixed CBK snapshot)
...
BEGIN
  ...
  -- Proposed entitlements: deterministic, per-user/scope/key/source ---
  -- Sources of proposed grants:
  --   subs_active_lifetime (Premium/Ultimate active)         -> library
  --   subs_active_basic                                     -> collection chatgpt_prompts
  --   subs_active_standard                                  -> collection {chatgpt,midjourney}
  --   cancelled_lifetime                                    -> flagged, no grant
  --   expired_subs                                          -> historical, no grant
  WITH s AS (
    SELECT us.user_id,
           lower(COALESCE(pl.tier,'unknown'))     AS tier,
           COALESCE(pl.is_lifetime,false)          AS is_lifetime,
  ...
  -- PayPal transactions (STRICT: gateway must be paypal) --------------
  WITH pp AS (
    SELECT t.*,
  ...
  -- UPayments transactions (both interpretations, no policy chosen) ---
  WITH up AS (
    SELECT t.*
      FROM public.transactions t
  ...
  -- Per-user credits under each interpretation, capped at threshold
  per_user_pp AS (
    SELECT t.user_id,
    ...
    'threshold_users', jsonb_build_object(
      ...
      'ambiguous_outcome_users',              COALESCE((
        SELECT count(*) FROM (
          ...
          EXCEPT SELECT user_id FROM reach_usd
          UNION
          SELECT user_id FROM reach_usd
          EXCEPT SELECT user_id FROM reach_kwd
        ) diff
      ),0)
    ),
    'duplicate_provider_reference_groups', (
  ...
  -- Confirmed PayPal-only credit rollup (deterministic, capped) --------
  WITH pp_paid AS (
    SELECT t.user_id,
           LEAST(SUM(ROUND(COALESCE(t.amount_usd,0) * v_rate))::bigint, v_threshold)::bigint AS credit_fils
      FROM public.transactions t
  ...
  -- Anomalies (read-only aggregates only) ------------------------------
  SELECT jsonb_build_object(
  ...
  -- Grant/Import contract surfaced explicitly to caller ---------------
...
-- Preview RPC stays admin-only (SECURITY DEFINER guards with _v2_require_admin()).
