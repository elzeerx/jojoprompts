-- Add upgrade-related columns to transactions table
ALTER TABLE public.transactions 
ADD COLUMN upgrade_from_plan_id UUID REFERENCES public.subscription_plans(id),
ADD COLUMN prorate_amount NUMERIC DEFAULT 0,
ADD COLUMN is_upgrade BOOLEAN DEFAULT false;

-- Create function to calculate prorated upgrade cost
...
BEGIN
  ...
  -- Calculate remaining days for current subscription
  remaining_days := EXTRACT(days FROM (current_subscription_end - CURRENT_DATE));
