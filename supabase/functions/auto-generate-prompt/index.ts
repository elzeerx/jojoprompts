...
const logger = createEdgeLogger('AUTO_GENERATE_PROMPT');
...
- Include relevant constraints and parameters`;
    ...
    if (!generatedPrompt || generatedPrompt.trim().length === 0) {
      logger.info('Retrying with fallback model GPT-4.1');
      
      // Retry with GPT-4.1 as a fallback
      ...
      if (fallbackResponse.ok) {
        ...
        const fallbackPrompt = fallbackData.choices[0]?.message?.content;
        if (fallbackPrompt && fallbackPrompt.trim().length > 0) {
          logger.info('Fallback model generated content successfully');
          // Use the fallback result
          data.choices[0].message.content = fallbackPrompt;
        } else {
          throw new Error('No content generated by either model');
        }
      } else {
        throw new Error('No content generated by primary model and fallback failed');
      }
    }
